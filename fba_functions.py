...
def set_PA(model, ptot, A_dict, O_frac):
    '''
    ...
    O_frac: the mass fraction of proteins not in ACTE sectors (default value=0.5953).
    '''
    sigma=0.5
    ...
    PA = model.problem.Constraint( expression = a_sector + c_sector,
                        name = 'PA', lb= 0, ub = ptot - O_frac*ptot)
    model.add_cons_vars([ PA ])
    return None

def pcfba(model, ptot, O_frac, NGAM, AA_lb, Glc_lb, OUR_lb, A_dict):
    '''
...
def dpcfba(model, ptot, O_frac, NGAM, Dr, A_dict, ic, T):
    '''
    ...
    for i in range(len(times)-1):
        ...
        fluxes = pcfba(model, ptot, O_frac, NGAM, AA_lb, Glc_lb, OUR_lb, A_dict)
